...
function LinkPopup({setLinkPopupOpen, curId, files, listening, setListening, fetchAll}) {
    ...
    const SaveButton = async (e) => {
      // e.preventDefault();
      console.log("FORM NAME", formName, formURL);
        ...
        <div ref={menuRef} className="bg-white absolute w-[30rem] h-72 left-2/4 top-2/4 rounded-lg z-50 -translate-x-1/2 -translate-y-1/2">
          <form onSubmit={SaveButton}>
            ...
            <div className="absolute bottom-0 right-0">
              <button onClick={CancelButton} type="button" className="bg-white border-gray-300 border-[1px] rounded-md w-16 h-8 mr-4 text-blue-600 font-bold text-sm hover:bg-blue-50">Cancel</button>
              <button onClick={SaveButton} type='submit' className="bg-blue-700 rounded-md w-16 h-8 mr-4 mb-4 text-white font-semibold text-sm hover:bg-blue-500">Save</button>
            </div>
          </form>
        </div>
      </div>
